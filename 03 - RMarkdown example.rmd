...
drive_filtered <- sqlQuery(myConn, "SELECT DISTINCT ServerName,
                                           CASE LEN(Drive)
	                                           WHEN 1 THEN Drive
		                                           ELSE SUBSTRING(Drive, 4, 1)
                                             END AS Drive,
                                           DriveTotalSpaceMB, DriveFreeSpaceMB, DriveFreeSpacePct, PollDate
                                    FROM DBInfo
                                    WHERE Servername IN ('DCICHISQL1\\MISDB', 'DCICHISQL2\\MISDB', 'DCICORSQL3\\MISDB', 'DCICORSQL4\\MISDB', 'DCICORSQL8\\REGION5') 
                                    AND PollDate >= DATEADD(year, -1, GETDATE())")
close(myConn)
...
db_plot1 <- db_filtered %>%
  ...
  arrange(PollDate) %>%
  filter(row_number()==1 | row_number()==n()) %>% 
  select(PollDate, DBName, DBUsedSpaceGB) %>%
  spread(DBName, DBUsedSpaceGB)  
...
ggplot(data = db_plot2, aes(x = PollDate, y = DBUsedSpaceGB, group = DBName )) + 
  geom_line(aes(color = DBName)) 
